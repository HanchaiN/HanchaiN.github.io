...
type _ArrayOf<T extends unknown[]> = T extends [
  infer T0,
  ...infer _T extends unknown[],
]
  ? [T0[], ..._ArrayOf<_T>]
  : [];
 ...
 */
export function zip<T extends unknown[]>(...arrays: _ArrayOf<T>): T[] {
  if (arrays.length === 0) return [];
  const minLength = Math.min(...arrays.map((arr) => arr.length));
  return Array.from(
    { length: minLength },
    (_, i) => arrays.map((arr) => arr[i]!) as T,
  );
}
