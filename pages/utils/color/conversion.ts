...
import type { TMatrix } from "../math/matrix.js";
import { constrain } from "../math/utils.js";
import type { TVector } from "../math/vector.js";
...
export const STD_ILLUM: XYZColor = [
  0.9504492182750991, 1.0, 1.0889166484304715,
];
...
const _OKLAB2LMS: TMatrix<number, 3, 3> = [
  [+1.0, +0.3963377774, +0.2158037573],
  [+1.0, -0.1055613458, -0.0638541728],
  [+1.0, -0.0894841775, -1.291485548],
];
const _LMS2OKLAB: TMatrix<number, 3, 3> = [
  [+0.2104542553, +0.793617785, -0.0040720468],
  [+1.9779984951, -2.428592205, +0.4505937099],
  [+0.0259040371, +0.7827717662, -0.808675766],
];
function oklab2lms(lab: OKLABColor): LMSColor {
  const lms_: TVector<number, 3> = [
    lab[0] * _OKLAB2LMS[0][0] +
      lab[1] * _OKLAB2LMS[0][1] +
      lab[2] * _OKLAB2LMS[0][2],
    lab[0] * _OKLAB2LMS[1][0] +
      lab[1] * _OKLAB2LMS[1][1] +
      lab[2] * _OKLAB2LMS[1][2],
    lab[0] * _OKLAB2LMS[2][0] +
      lab[1] * _OKLAB2LMS[2][1] +
      lab[2] * _OKLAB2LMS[2][2],
  ];
...
const XYZ2LMS: TMatrix<number, 3, 3> = [
...
// CIE RGB
// const XYZ2RGB: TMatrix<number, 3, 3> = [
...
// ];
// const RGB2XYZ: TMatrix<number, 3, 3> = [
//   [0.49, 0.31, 0.2],
...
const RGB2LMS: TMatrix<number, 3, 3> = [
  [0.4122214708, 0.5363325363, 0.0514459929],
  [0.2119034982, 0.6806995451, 0.1073969566],
  [0.0883024619, 0.2817188376, 0.6299787005],
];
const LMS2RGB: TMatrix<number, 3, 3> = [
...
>(
  ...
  const q = new PriorityQueue(([dist]: [dist: number, ind: number]) => dist);
  dist[i] = 0;
  q.push([0, i]);
  while (q.top() !== null) {
  ...
  for (
    let at = o;
    at !== -1 && typeof prev[at] !== "undefined" && prev[at] !== -1;
    at = prev[at]!
  ) {
