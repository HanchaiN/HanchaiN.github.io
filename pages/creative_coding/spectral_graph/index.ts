...
export default function execute() {
  ...
  function project<T extends TVector<number>[], N extends number>(
    vs: T,
    n: N,
  ): TVector<number, N>[] {
    ...
    const variance = new Array(vs[0]!.length)
      .fill(0)
      .map((_, i) =>
        average(vs.map((v) => Math.pow(v[i]! - center[i]!, 2), 0)),
      );
    return vs.map(
      (vec) =>
        vec
          ...
          .sort((a, b) => b.p - a.p)
          .slice(0, n)
          .map((y) => y.v) as readonly number[] as TVector<number, N>,
    );
  }
