import fft from "@/utils/algo/fft.js";
      ...
      kspace.forEach((row, i) =>
        row.forEach((val, j) => {
          const x =
            ...
            y =
              j <= row.length / 2
                ? j + Math.ceil(row.length / 2 - 1)
                : j - Math.ceil((row.length + 1) / 2);
          const value = symlog(Complex.abs(val));
