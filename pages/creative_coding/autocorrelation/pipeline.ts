import { average } from "@/utils/math/utils.js";
...
export function rescale_yin(buffer: Float32Array) {
  const acc = buffer
    .reduce<number[]>((a, b) => [...a, (a.at(-1) ?? 0) + b], [])
    .map((v) => (v > 1e-5 ? v : 1));
  return buffer.map((v, i) => (v * i) / acc[i]!);
}
...
export function extractPeriod(peaks: number[]) {
  ...
  return average(
    periods.splice(
      Math.floor(periods.length * 0.3),
      Math.ceil(periods.length * 0.7),
    ),
  );
}
...
export function detectPitchYIN(buf: Float32Array, sampleRate: number) {
  ...
  if (bestTau === -1) {
    ...
    if (minVal >= hardThreshold) return { freq: -1, confidence: 0 };
  }
  ...
  return { freq: sampleRate / T0, confidence: 1 - S0 };
}
