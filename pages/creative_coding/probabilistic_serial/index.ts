...
import type { HCLColor } from "@/utils/color/conversion.ts";
import { getPaletteBaseColor } from "@/utils/color/palette.js";
import { DragListY } from "@/utils/dom/element/DragListY.js";
  ...
  cache.setGen(
    ...
    function _getBaseDistribution(
      ...
      while (!leftover.values().every((v) => v.compare() <= 0)) {
        ...
        const stepsize = prefs.reduce((min, [_agent, items]) => {
          while (items.length > 0 && leftover.get(items[0]!)!.compare() <= 0) {
            items.shift();
          }
          if (items.length === 0) return min;
          const item = items[0]!;
          share.set(item, (share.get(item) || 0) + 1);
          const step = Fraction.div(
            leftover.get(item)!,
            new Fraction(share.get(item)!),
          );
