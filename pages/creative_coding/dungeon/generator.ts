import { PriorityQueue } from "@/utils/algo/datastructure.js";
import { delaunay_triangulation_step } from "@/utils/algo/delaunay_triangulation.js";
import { minimum_spanning_tree_step } from "@/utils/algo/minimum_spanning_tree.js";
import { PerlinNoise } from "@/utils/math/noise.js";
import { randomGaussian } from "@/utils/math/random.js";
import { constrain, map } from "@/utils/math/utils.js";
import { vector_magSq, vector_sub, type TVector } from "@/utils/math/vector.js";
...
export function drawDungeon(
  ...
  data.nodes.forEach((p) => {
    ...
    ctx.beginPath();
    ctx.arc(unit.x * p[0], unit.y * p[1], 1, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  data.edges?.forEach(([ia, ib], i) => {
    ...
    const a = data.nodes[ia]!,
      b = data.nodes[ib]!;
    ctx.beginPath();
    ctx.moveTo(unit.x * a[0], unit.y * a[1]);
    ctx.lineTo(unit.x * b[0], unit.y * b[1]);
    ctx.stroke();
  });
}

export class DungeonGenerator implements IDungeonState {
  ...
  addRoom(
    ...
    const room = {
      left: Math.round(node[0] - width / 2),
      right: Math.round(node[0] + width / 2),
      bottom: Math.round(node[1] - height / 2),
      top: Math.round(node[1] + height / 2),
  ...
  *genEdges_Stepwise() {
    const edges_gen = delaunay_triangulation_step(this.nodes, [
      [0, 0],
      [2 * this.GRID_SIZE.x, 0],
      [0, 2 * this.GRID_SIZE.y],
    ]);
    while (true) {
  ...
  *genTree_Stepwise() {
      ...
      this.edges.map(
        ([ia, ib]) =>
          [
            ia,
            ib,
            vector_magSq(vector_sub(this.nodes[ia]!, this.nodes[ib]!)),
  ...
  _estimateCost(from: TVector<number, 2>, to: TVector<number, 2>) {
    let dist = 0;
    const steep = Math.abs(to[1] - from[1]) > Math.abs(to[0] - from[0]);
  ...
  *generate() {
    ...
    for (const [i_begin, i_target] of this.edges) {
      const nodes = this.grid
        .map((_, i) => _.map((_, j) => [i, j] as TVector<number, 2>))
        .flat(1);
      ...
      const gen = shortest_path(
        ...
        nodes.findIndex(
          (v) =>
            v[0] === Math.round(this.nodes[i_begin]![0]) &&
            v[1] === Math.round(this.nodes[i_begin]![1]),
        ),
        nodes.findIndex(
          (v) =>
            v[0] === Math.round(this.nodes[i_target]![0]) &&
            v[1] === Math.round(this.nodes[i_target]![1]),
        ),
        (a, b) => a[0] === b[0] && a[1] === b[1],
        (a) => this._getCost(a[0], a[1]),
        (a, b) => this._estimateCost(a, b),
        (a) => [
          nodes.findIndex(
            (v) =>
              v[0] === Math.round(a[0] + 1) && v[1] === Math.round(a[1] + 0),
          ),
          nodes.findIndex(
            (v) =>
              v[0] === Math.round(a[0] - 1) && v[1] === Math.round(a[1] + 0),
          ...
          nodes.findIndex(
            (v) =>
              v[0] === Math.round(a[0] + 0) && v[1] === Math.round(a[1] + 1),
          ),
          nodes.findIndex(
            (v) =>
              v[0] === Math.round(a[0] + 0) && v[1] === Math.round(a[1] - 1),
        ...
        (a, isFinal) =>
          (grid_[Math.round(a[0])]![Math.round(a[1])]! = isFinal
            ? GRID_STATE.PATH
            : GRID_STATE.SEARCH_PATH),
        (a) =>
          (grid_[Math.round(a[0])]![Math.round(a[1])]! =
            GRID_STATE.SEARCH_CURR),
        () =>
          (grid_ = grid_.map((_) =>
            _.map((_) =>
